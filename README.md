...
Family Health Agent 是面向家庭长期使用的健康档案、家庭健康共享与受控 Agent 辅助系统。项目当前进入 **Phase 12.A：Auth/JWT 与用户会话地基**：后端核心业务 API、权限闭环、API 安全、Agent Harness、Agent Tool Executor、受控 Agent workflow、Agent API 最小入口、移动端 MVP 演示闭环和 LLM provider 受控验证已经可验证运行；当前阶段开始补齐真实登录、会话和 token 基础能力。
...
- Phase 12.A 已新增 Auth/JWT 与用户会话地基：密码哈希、登录、刷新、登出、`/auth/me` 和 demo 用户密码 seed。
...
- 当前不是完整产品，仍缺少全局 JWT 迁移、生产级登录策略、其他 workflow LLM 接入、LangGraph、OCR/upload/RAG、正式上传、生产部署和完整真机视觉 QA。
- 当前阶段为 **Phase 12.A：Auth/JWT 与用户会话地基**。默认配置下 `AUTH_ENABLED=false`，既有 `X-Current-User-Id` demo header 仍保留，后续 Phase 12.B 再做 current user 依赖迁移。
...
手机真机访问电脑上的 FastAPI 不能使用 `localhost` 或 `127.0.0.1`，需要配置电脑局域网 IP，例如 `http://192.168.x.x:8000`。Phase 12.A 已新增最小 Auth/JWT 入口，但移动端和既有业务 API 当前仍使用开发调试 header `X-Current-User-Id`；全局迁移将在后续 Phase 12.B 处理。
...
> 进度说明：Phase 12 的实际执行顺序已调整为先补齐 Auth/JWT 与用户会话边界，再继续推进 LangGraph。当前完成的是 Phase 12.A，不代表所有 API 已切换到 JWT。
...
## Phase 12.A Auth/JWT Foundation

Phase 12.A 已新增最小 Auth/JWT 与用户会话地基。

- 默认 `AUTH_ENABLED=false`，`AUTH_DEMO_LOGIN_ENABLED=true`。
- 既有 `X-Current-User-Id` demo header 仍保留，未强制替换所有业务 API。
...
- Phase 12.B 建议继续做 current user dependency 迁移与 demo header 收口策略。
