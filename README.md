...
Family Health Agent 是面向家庭长期使用的健康档案、家庭健康共享与受控 Agent 辅助系统。项目当前进入 **Phase 10.B：daily_health_brief 可选接入 LLM**：后端核心业务 API、权限闭环、API 安全、Agent Harness、Agent Tool Executor、受控 Agent workflow、Agent API 最小入口、移动端 MVP 演示闭环已经可验证运行，LLM 底座已开始受控接入。
...
- 当前阶段为 **Phase 10.B：daily_health_brief 可选接入 LLM**。默认配置下 `daily_health_brief` 仍走规则简报。
...
Phase 10.A 完成 LLM Client 底座。Phase 10.B 只把 LLM 可选接入 `daily_health_brief`：
...
- LLM 输出必须经过 Safety Policy；配置错误、调用失败、超时、空输出或安全拦截都会回退规则简报。
