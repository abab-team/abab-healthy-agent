...
def run_chat_health_query(
    ...
    *,
    ...
    if plan.metric_type == "bmi":
        results = _execute_bmi_tools(execution_context, executor, days=plan.time_range.days)
        answer = _compose_insight_answer(
            plan,
            results,
            conversation_responder=conversation_responder,
...
def _execute_bmi_tools(
    context: AgentWorkflowContext,
    executor: AgentToolExecutor,
    *,
    days: int,
) -> list[ToolExecutionResult]:
    """Use two separately permission-gated facts; BMI is derived in Agent insight only."""
    calls = (
        ("health_profile.get", {}),
        ("health_data.metric.summary", {"metric_type": "weight", "days": days, "aggregation": "latest"}),
    )
    return [_execute_tool(context, executor, tool_name, input_data) for tool_name, input_data in calls]
