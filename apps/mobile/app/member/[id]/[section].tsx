...
export default function MemberArchiveSectionScreen() {
  ...
  const timelineItems: ArchiveTimelineItem[] = safeSection === "metrics" ? (data?.bloodPressure ?? []).map((item) => ({ date: formatDate(item.recorded_at), detail: `${item.systolic}/${item.diastolic} mmHg`, icon: "pulse-outline", id: item.id, title: "血压记录", tone: theme.colors.primary })) : safeSection === "records" ? [
    ...
    ...(data?.metrics ?? []).map((item) => ({ date: formatDate(item.measured_at), detail: item.value_numeric === null || item.value_numeric === undefined ? item.value_text ?? "已记录" : `${item.value_numeric}${item.unit ? ` ${item.unit}` : ""}`, icon: "analytics-outline" as const, id: `metric-${item.id}`, title: `${({ heart_rate: "心率", sleep_duration: "睡眠", steps: "步数", temperature: "体温", weight: "体重" } as Record<string, string>)[item.metric_type] ?? "健康指标"}记录`, tone: theme.colors.primary })),
  ...
  ].sort((left, right) => new Date(right.date).getTime() - new Date(left.date).getTime()) : safeSection === "documents" ? [
