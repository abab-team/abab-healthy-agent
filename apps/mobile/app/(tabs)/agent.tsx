  ...
  async function runChatQuery(question = query) {
    ...
    if (result.ok && result.data) {
      ...
      setMessages((items) => [
        ...
        {
          content: run.generated_content ?? run.message ?? "系统内暂时没有可展示的回答。",
    ...
    <View>
      <View style={styles.composer}>
        <TextInput
          ...
          placeholder="发送健康记录相关问题…"
          placeholderTextColor={theme.colors.subtle}
          style={styles.input}
          value={query}
        />
        <Pressable disabled={queryLoading} onPress={() => void runChatQuery()} style={[styles.sendButton, queryLoading ? styles.sendButtonDisabled : null]}>
          <Ionicons color="#FFFFFF" name="arrow-up" size={20} />
        </Pressable>
      </View>
      <Text style={styles.composerHint}>{queryLoading ? "正在整理系统内记录…" : "仅支持受控健康记录查询，不提供诊断或处方建议。"}</Text>
    </View>
  );

  return (
    <AppScreen footer={composer}>
      <ScreenHeader subtitle="你的私人健康记录整理助手。" title="AI 健康管家" />

      <View style={styles.questionHeader}>
        <Text style={styles.sectionTitle}>你可以这样问</Text>
        <Text style={styles.sectionCaption}>连续追问会沿用当前对话上下文</Text>
      </View>
      <View style={styles.suggestions}>
        {suggestions.map((suggestion) => (
          <Pressable key={suggestion} onPress={() => void runChatQuery(suggestion)} style={styles.suggestion}>
            <Text style={styles.suggestionText}>{suggestion}</Text>
            <Ionicons color={theme.colors.primaryDark} name="arrow-up-outline" size={15} />
          </Pressable>
        ))}
      </View>
      ...
      <View style={styles.chatList}>
        {messages.length === 0 ? (
          <>
            <ChatBubble content="你好，Gala 👋\n\n我可以帮你整理健康记录、查看趋势、准备就医资料。" role="assistant" />
            <InlineSafetyNotice />
          </>
        ) : (
          messages.slice(-8).map((message) => (
            <View key={message.id} style={styles.messageGroup}>
              ...
              {message.role === "assistant" ? <InlineSafetyNotice /> : null}
            </View>
          ))
        )}
      </View>
      ...
      {queryResult ? (
        <Link href={routes.agentRun(queryResult.trace_id)} style={styles.runLink}>
          查看本次整理记录
        </Link>
      ) : null}
...
const styles = StyleSheet.create({
  ...
  suggestion: { alignItems: "center", backgroundColor: theme.colors.tealSoft, borderRadius: theme.radius.pill, flexDirection: "row", gap: 7, paddingHorizontal: 12, paddingVertical: 9 },
  suggestionText: { color: theme.colors.primaryDark, fontSize: 12, fontWeight: "800" },
  suggestions: { alignItems: "flex-start", flexDirection: "row", flexWrap: "wrap", gap: 8 },
});
