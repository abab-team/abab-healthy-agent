...
export default function SettingsScreen() {
  ...
  return <AppScreen>
    ...
    <CardBase style={styles.profileCard}>
      ...
      <View style={styles.profileCopy}><View style={styles.nameRow}><Text style={styles.name}>{currentUser.name}</Text><StatusBadge label="当前用户" tone="mint" /></View><Text style={styles.profileText}>记录每一天，守护家人的健康生活。</Text><Text style={styles.familyText}>当前家庭：{currentUser.familyName}</Text></View>
    </CardBase>
    ...
    <CardBase style={styles.moreCard}>
      ...
      {moreOpen ? <View style={styles.moreContent}>
        ...
        {debugOpen ? <View style={styles.debugContent}><BackendStatusCard accessTokenPreview={session.authSession.accessTokenPreview} apiBaseUrl={session.apiBaseUrl} authMode={session.authMode} currentUserId={session.currentUserId} health={health.data} healthError={health.error} loading={health.loading} mode={session.dataMode} onRefresh={() => void health.reload()} warnings={session.warnings} /><View style={styles.debugActions}><Pressable onPress={() => router.push("/login")} style={styles.outlineButton}><Text style={styles.outlineText}>登录设置</Text></Pressable><Pressable onPress={() => void session.authSession.logout()} style={styles.outlineButton}><Text style={styles.outlineText}>退出登录</Text></Pressable></View></View> : null}
      </View> : null}
    </CardBase>
...
const styles = StyleSheet.create({
  ...
  debugActions: { flexDirection: "row", flexWrap: "wrap", gap: 10 },
  debugContent: { gap: 12 },
